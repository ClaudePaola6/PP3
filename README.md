...
version="1.1"
 ...
 <!-- Straight horizontal line (X-axis) -->
 <line x1="-300" y1="0" x2="300" y2="0" stroke="red" stroke-width="2" />

 <!-- Circle centered at (0, 0) -->
 <circle cx="0" cy="0" r="100" fill="lightblue" stroke="black" stroke-width="2" />

 <!-- Rectangle to the right of the circle -->
 <rect x="120" y="-50" width="100" height="100" fill="lightgreen" stroke="black" stroke-width="2" />
...
<details>
     <code>#</code> erzeugt eine Überschrift erster Ordnung (H1)
     <code>#</code><code>#</code> erzeugt eine Überschrift zweiter Ordnung (H2)
     <code>#</code><code>#</code><code>#</code> erzeugt eine Überschrift dritter Ordnung (H3) und so weiter, bis <code>#</code><code>#</code><code>#</code><code>#</code><code>#</code><code>#</code>` für eine 
      H6-Überschrift
</details> 
......
</details>
...
Für italic: man benutzt jeweils ein Sternchen oder ein Unterstricht vor und nach dem geschriebenen Wort. ZB: 

     *Ananas* oder _Ananas_
Für bold: man benutzt jeweils zwei Sternchen oder zwei Unterstrichte vor und nach dem geschriebenen Wort. ZB: 

     **Ananas** oder __Ananas__
</details>
<details>
...
<details>
    ...
    Beim Kompilieren mit `pdflatex` werden mehrere Dateien erzeugt:

    - `example.pdf` – das fertige PDF-Dokument
    - `main.aux` – enthält Metadaten und Querverweise, wird bei weiteren Kompilierungen benötigt
    - `main.log` – ein Protokoll der Kompilierung, nützlich zur Fehlersuche
    - Optional: weitere Dateien wie `.out`, `.toc` oder `.bbl`, je nach verwendeten Paketen oder Befehlen

    Diese Dateien sind wichtig für weiterführende LaTeX-Funktionen wie Inhaltsverzeichnisse, Literaturverzeichnisse oder automatische Referenzen.
</details>

<details>
    <summary>How do you change the name of the pdf-file?</summary>

    Der Name der erzeugten PDF-Datei kann mit der Option `-jobname=` geändert werden.  
    Beispiel:

    ```sh
    pdflatex -jobname=neuer_name main.tex
    ```

    Dadurch wird eine Datei namens `neuer_name.pdf` erzeugt, obwohl die Quelltextdatei `main.tex` heißt.  
    Hinweis: Hilfsdateien wie `.aux` und `.log` behalten trotzdem den Namen der Quelltextdatei (`main.aux`, `main.log`).
</details>
...
<details>
    <summary>What changes in your pdf, if you change the documentclass to <code>book</code>änderst?</summary>

    Wenn du `\documentclass{book}` statt `article` verwendest:

    - Das Layout wird für längere Dokumente wie Bücher angepasst.
    - Du kannst `\chapter{}` verwenden (nicht in der `article`-Klasse verfügbar).
    - Kapitel beginnen auf neuen (rechten) Seiten.
    - Seitenzahlen und Abschnittsnummerierungen ändern sich.
    - Es wird standardmäßig eine leere Seite zwischen Kapiteln eingefügt (einseitiger Druck).

    Insgesamt wirkt das PDF professioneller und strukturierter für große Dokumente.
</details>

<details>
    <summary>What changes in your pdf, if you add <code>\section{Intro}</code> after <code>\maketitle</code></summary>

    Wenn du `\section{Intro}` nach `\maketitle` einfügst:

    - Es wird ein Abschnitt mit der Überschrift „Intro“ eingefügt.
    - Dieser Abschnitt wird automatisch nummeriert (z. B. „1 Intro“).
    - Der Text wird thematisch strukturiert.
    - Falls du ein Inhaltsverzeichnis (`\tableofcontents`) verwendest, erscheint der Abschnitt dort ebenfalls.

    Dies ist der empfohlene Weg, um längere Dokumente zu gliedern.
</details>
